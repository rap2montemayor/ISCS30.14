...
const EntityList ECS::queryComponents(const ComponentList& components) {
    ...
    auto itr = std::remove_if(entities.begin(), entities.end(), notInAllComponents);
    entities.resize(std::distance(entities.begin(), itr));

    return entities;
}

void ECS::markForDeletion(const EntityID& id) {
    toDelete.insert(id);
}
